    ...
    <Section
      title={
        <span className="flex items-center gap-2">
          Live viz
          <span className={cn(
            "rounded-md px-1.5 py-0.5 text-[10px] font-normal normal-case tracking-normal",
            connected ? "bg-ok/15 text-ok" : "bg-danger/15 text-danger",
          )}>
            {connected ? "live" : "disconnected"}
          </span>
        </span>
      }
      hint={state.running_scene ? <span className="text-accent">▶ {state.running_scene}</span> : <span>— idle —</span>}
    >
      <Stage>
        {fixtures.map((f) => {
          ...
          if (f.type === "atomic") {
            return <AtomicDot key={f.id} p={p} a={state.atomic} />;
          }
          if (f.type === "fog") {
            return <FogCloud key={f.id} p={p} level={state.fog} />;
          }
          return null;
        })}
      </Stage>
  ...
  return (
    <div
      ...
      style={{
        aspectRatio: "16 / 10",
        background:
          "radial-gradient(ellipse at 50% 14%, rgba(108,142,255,0.06) 0%, rgba(108,142,255,0) 30%), radial-gradient(ellipse at 50% 90%, rgba(255,80,80,0.04) 0%, rgba(255,80,80,0) 25%), #050507",
      }}
    >
      <span className="absolute left-1/2 top-1.5 -translate-x-1/2 text-[9px] tracking-[0.3em] text-white/15">CROWD</span>
      <span className="absolute left-1/2 bottom-1.5 -translate-x-1/2 text-[9px] tracking-[0.3em] text-white/15">STAGE</span>
      {children}
    </div>
  );
}
...
function TriparDot({ f, p, t }: { f: FixtureMeta; p: { x: number; y: number }; t?: TriparState }) {
  ...
  const lit = t.strobe > 0 ? 1 : 0;            // firmware quirk
  const w = t.white;
